...
public class Board {
    ...
    public void setCardTeamsAndFillScoreBoard (CardTeam[][] map, Scoreboard scoreboard) {

        int longWordLength =  0;
        for (int z = 0; z < 5; z++) {
            for (int y = 0; y < 5; y++) {

                Card currentCard = board[z][y];
                CardTeam currentCardTeam = currentCard.setCardTeam(map[z][y]);

                if (longWordLength < currentCard.getWord().length()){
                    longWordLength = currentCard.getWord().length();
                }

                switch (currentCardTeam) {

                    case BLUE:

                        scoreboard.getBlueWords().add(currentCard);
                        break;

                    case RED: scoreboard.getRedWords().add(currentCard);
                        break;

                    case CIVILIAN:
                        scoreboard.getCivilianWords().add(currentCard);
                        break;

                    case ASSASSIN:
                        scoreboard.getAssassin().add(currentCard);
        ...
        scoreboard.fillListsWithEmptyStrings();
        scoreboard.setLongestWordLength(longWordLength);
